...
const bridge = (overrides: Record<string, unknown> = {}) => ({ amount: "1", token: "USDC", state: "success", provider: "cctp-v2", source: { address: recipient, chain: { type: "evm", chainId: 84_532 } }, destination: { address: recipient, recipientAddress: recipient, useForwarder: false, chain: { type: "evm", chainId: ARC_CHAIN_ID } }, steps: [{ name: "Burn", state: "success", txHash: burnHash }, { name: "FetchAttestation", state: "success", data: { message: "0x1234", eventNonce: "101", attestation: "0xabcd", status: "complete" } }, { name: "Mint", state: "success", txHash: mintHash, forwarded: false }], ...overrides }) as never;

describe("settlePaymentRequest", () => {
  it("settles and correlates the real Base Sepolia to Arc proof fixture", () => expect(settlePaymentRequest({ request, bridgeResult: bridge(), destinationReceipt: receipt() })).toMatchObject({ paymentState: "settled", mintTransactionHash: mintHash, amountBaseUnits: 1_000_000n, correlation: { obligation: { id: "INV-REAL-1" }, source: { burnTransactionHash: burnHash }, destination: { mintTransactionHash: mintHash } } }));
  it.each([
