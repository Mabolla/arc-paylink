...
The current walletless mainnet pilot is under product-hardening. Its exact authorization, secret-handling, encrypted creator-recovery, PII, and remaining release boundaries are documented in [the security and privacy threat model](./docs/security-privacy-threat-model.md). A private claim link is a bearer secret; the current escrow is not cryptographically bound to an email address.
